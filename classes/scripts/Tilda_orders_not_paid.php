...
class Tilda_orders_not_paid extends Script
{
	...
	private function telegramChannel()
	{
		try {
			...
			");
			if (empty($telegraChannel)) {
				throw new \Exception("channel (name: orders_unpaid, city_id: {$this->orderData['city_id']}, shop_crm_id: {$this->orderData['shop_crm_id']}) not found in DB");
			}
